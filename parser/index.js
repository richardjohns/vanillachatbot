...
let currentWeather = response => {
  if (response.query.results) {
    ...
    return `Right now, ${getPrefix(code)} ${text.toLowerCase()} in ${
      location
    }. It is ${getFeel(Number(temp))} at ${temp} degrees Celsius.`;
  } else {
...
let forecastWeather = (response, data) => {
  if (response.query.results) {
    ...
    return `${testConditions ? "Yes" : "No"}, ${getPrefix(
      getForecast.code,
      "future"
    )} ${getForecast.text} ${data.time} in ${location}`;
  } else {
    return "I don't seem to know anything about this place...Sorry :(";
  }
};
