...
class FBeamer {
  constructor(config) {
    try {
      if (
        !config ||
        config.PAGE_ACCESS_TOKEN === undefined ||
        config.VERIFY_TOKEN === undefined
      ) {
  ...
  registerHook(req, res) {
    ...
    console.log("This is req.query['hub.mode']: ", req.query['hub.mode']);
  ...
  // resubscribes to fb page after server restarts
  subscribe() {
    request(
      {
